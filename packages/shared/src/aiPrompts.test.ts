...
describe("buildImportEnrichPrompt / parseImportEnrichResponse", () => {
  ...
  it("sends validated facts, bounded history evidence, and only current entity ids", () => {
    ...
    expect(JSON.stringify(user)).toContain("LINDEN MARKET 123");
...
describe("runImportRecognitionPipeline", () => {
  ...
  describe("historical metadata regression", () => {
    ...
    it("leaves a conflicting historical category for review even when the model selects one", async () => {
      const result = await recognizeBakery({
        records: [bakeryHistory(), bakeryHistory({ category: "Dining" })],
        modelCategory: "category-1",
      });
      expect(result.proposals[0]).toMatchObject({ categoryId: null, name: "Bread", envelopeId: "envelope-1" });
      expect(result.proposals[0]!.reviewReasons).toContain("history_conflict");
    });
  ...
  it.each(["default", "durable"] as const)("fills only the missing category of a known purchase in %s mode", async (pipelineMode) => {
    // given: merchant history identifies bread and its envelope, but never supplied a category
    const requests: ChatRequest[] = [];
    const result = await runImportRecognitionPipeline({
      ...base,
      pipelineMode,
      categories: [{ id: "groceries", name: "Groceries", archived: false }],
      historyRecords: [{ ...history("account-1", "Food"), name: "Bread" }],
      chat: async (request) => {
        requests.push(request);
    ...
    expect(requests).toHaveLength(2);
    const context = JSON.parse(requests[1]!.messages[1]!.content as string);
    expect(context.rows[0].historyCandidates[0]).toMatchObject({ name: "Bread", category: null });
    expect(context.entities.categories).toEqual([{ id: "groceries", name: "Groceries" }]);
  });

  it("excludes archived categories from the prompt and rejects their model-selected ids", async () => {
    const requests: ChatRequest[] = [];
    const result = await runImportRecognitionPipeline({
      ...base,
      categories: [
        { id: "active", name: "Groceries", archived: false },
        { id: "archived", name: "Old groceries", archived: true },
      ],
      chat: async (request) => {
        requests.push(request);
        if (requests.length === 1) return extracted();
    ...
    expect(result.proposals[0]!.categoryId).toBeNull();
    expect(JSON.parse(requests[1]!.messages[1]!.content as string).entities.categories).toEqual([{ id: "active", name: "Groceries" }]);
  });

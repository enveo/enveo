...
describe("input focus presentation", () => {
  test("moves the focus indicator from the rectangular input to its rounded shell", () => {
    ...
    const shellRule = `.${INPUT_FOCUS_CLASS}:has(>input:focus-visible){outline:2px solid var(--focus-ring);outline-offset:2px}`;
  ...
  test("uses only an underline for the transaction name field", () => {
    expect(NAME_UNDERLINE_FOCUS_CSS).toBe(`.${NAME_UNDERLINE_FOCUS_CLASS}:focus-visible{outline:none;box-shadow:inset 0 -2px 0 var(--focus-ring)}`);

/**
 * Server↔BYOK parity of the AI transport timeout budget.
 *
 * The two transports (packages/api/src/openaiHttp.ts — operator key;
 * packages/web/src/lib/openai.ts — byok + the /api/ai mirror) once each kept a
...
describe("AI transport timeout parity (api ↔ web via @enveo/shared)", () => {
  ...
  it("the web transport takes its caps from the shared constants", () => {
  ...
  it("the vision paths use the vision cap on both sides (import route ↔ byok extraction ↔ client cap)", () => {
    expect(read("packages/api/src/routes/import.ts")).toContain("AI_VISION_TIMEOUT_MS");
    expect(read("packages/web/src/lib/ai.ts")).toContain("AI_VISION_TIMEOUT_MS");
    expect(read("packages/web/src/lib/api.ts")).toContain("AI_IMPORT_EXTRACT_TIMEOUT_MS");
  });

  it("no transport file hardcodes a timeout amount — the shared module is the only place the numbers exist", () => {
    for (const rel of [...TRANSPORT_FILES, "packages/api/src/routes/import.ts", "packages/web/src/lib/ai.ts"]) {
      const src = read(rel);

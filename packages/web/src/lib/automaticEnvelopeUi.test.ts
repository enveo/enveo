...
describe("reconciliation transaction preparation", () => {
  it("preserves the entered actual balance when only the live account link changes", () => {
    // given: the user entered a positive adjustment while this account had its old link
    const opened = { id: "A-savings", balance: 100_00 };

    // when: a background refresh presents the same account and balance after a relink
    const refreshed = reconciliationActualValueAfterAccountRefresh("125", opened, { ...opened });

    // then: the live preview can reroute without erasing the amount being reviewed
  ...
  it("follows a live relink only for the untouched negative-adjustment default", () => {
    // given: the negative adjustment still uses the account-derived default
    const chosen = {
      accountId: "A-savings",
      automaticEnvelopeId: "E-savings",
    ...
    // when: that same account is relinked while the reconciliation sheet is open
    const refreshed = reconciliationEnvelopeAfterAccountRefresh(chosen, "A-savings", "E-travel");

    // then: the live account link supplies the new editable default

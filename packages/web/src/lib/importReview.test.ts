...
describe("screenshot import review view model", () => {
  ...
  it("keeps incomplete rows editable and unchecked, and applies a completed edit after rebuilding review", () => {
    ...
    const build = () => buildImportReviewRows({ recognition: result, ledger: ledger(), dryRunResults: [], automaticEnvelopeId: null, budgetCurrency: "USD" });
    const review = build();
    ...
    expect(chosen[0]).toMatchObject({ importRowId: "cropped", date: "2031-08-02", amount: 2500, rawPlace: "raw cropped\n$25.00" });
  ...
  it("requires correcting an impossible calendar date before an incomplete row can be added", () => {
    const rows = buildImportReviewRows({
      recognition: recognition(
        [row("invalid-date", { date: "2031-02-31" })],
        [proposal("invalid-date", { date: "2031-02-31", disposition: "unresolved", selected: false, reviewReasons: ["missing_fact"] })],
      ),
      ledger: ledger(),
      dryRunResults: [],
      automaticEnvelopeId: null,
    ...
    rows[0]!.include = true;
    expect(() => reviewedImportRowsForApply({ rows, edited: { 0: editedItem({ date: "2031-02-31" }) }, editedAutomaticDefaults: {} })).toThrow(
      "import_review_incomplete",
    );
    expect(reviewedImportRowsForApply({ rows, edited: { 0: editedItem({ date: "2031-02-28" }) }, editedAutomaticDefaults: {} })[0]?.date).toBe("2031-02-28");
  });
  ...
  it("keeps a reconciled exact duplicate truthful while leaving it unselectable and noneditable", () => {
    ...
    current.transactions.push({
      ...
      items: [],
      createdAt: "2031-08-02T00:00:00.000Z",
    });
    const validated = validateImportExtraction({ batch: { rows: [row("exact")] }, budgetCurrency: "USD" });
  ...
  it("keeps a currency warning advisory at the final apply boundary", () => {
    const review = buildImportReviewRows({
      recognition: recognition([row("currency", { currency: "EUR" })], [proposal("currency", { currency: "EUR" })]),
      ledger: ledger(),
      dryRunResults: [dryResult({ currency: "EUR" })],
      automaticEnvelopeId: null,
      budgetCurrency: "USD",
    });
...
describe("why nothing matches", () => {
  const txn = (over: Partial<Transaction>): Transaction => ({
    ...
    allocationToEnvelopeId: null,
    items: [],
    createdAt: "2031-08-20T00:00:00.000Z",
    ...over,
  });
  ...
  it("reports the doubtful rows' reach and the hand-entered rows of the period, newest first", () => {
    const diagnosis = importBalanceDiagnosis({
      ...
      transactions: [
        txn({ id: "imported", sourceRef: "EXAMPLE MARKET", date: "2031-08-25" }),
        txn({ id: "before", date: "2031-08-10", amount: 931 }),
        txn({ id: "in", type: "transfer", accountId: "checking-west", toAccountId: "checking-east", amount: 184275, date: "2031-08-25", name: null }),
        txn({ id: "out", type: "transfer", accountId: "checking-east", toAccountId: "checking-west", amount: 6729, date: "2031-08-22" }),
        txn({ id: "refund", isRefund: true, amount: 3176, date: "2031-08-14", note: "Balance adjustment" }),
        txn({ id: "elsewhere", accountId: "checking-west", date: "2031-08-24" }),
      ],
      accountId: "checking-east",
      since: "2031-08-14",
    });
    ...
    expect(diagnosis.manualEntries).toEqual([
      { id: "in", date: "2031-08-25", effect: 184275, name: null, transfer: true },
      { id: "out", date: "2031-08-22", effect: -6729, name: null, transfer: true },
      { id: "refund", date: "2031-08-14", effect: 3176, name: "Balance adjustment", transfer: false },
    ]);
  });

  it("finds the earliest dated row as the period start", () => {
    expect(importPeriodStart([{ date: null }, { date: "2031-08-29" }, { date: "2031-08-27" }])).toBe("2031-08-27");

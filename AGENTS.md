...
1. `cp .env.example .env` and set `POSTGRES_PASSWORD` (optionally `OPENAI_API_KEY` + `OPENAI_MODEL` for AI features).
2. `make up` (build + start; migrations and first-run seed check run automatically). A fresh install starts with an EMPTY budget — the in-app onboarding wizard does the initial setup. `make logs`, `make down`.
...
`db:seed` is a DEV-ONLY tool with an anonymized demo dataset. Never run it against data you care about. For an empty-start/onboarding test use `bun src/db/ensure-seed.ts` instead.
...
## Auth modes
- `AUTH_MODE=none` is the DEFAULT: no middleware, single budget from the DB — intended for private, network-perimeter deployments (VPN/private network).
- `AUTH_MODE=multi` (better-auth, Google) needs `BETTER_AUTH_SECRET` + `BETTER_AUTH_URL` (+ Google client creds); budgets are created lazily per user (empty → onboarding wizard). E2E in multi mode: `DEV_LOGIN=1` enables email+password sign-up (`POST /api/auth/sign-up/email`) — NEVER in production.

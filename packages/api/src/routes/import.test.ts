...
describe("screenshot import wire inputs", () => {
  it("projects only complete safely selected candidates into the legacy items envelope", () => {
    ...
    expect(legacyItemsFromRecognition({ rows, proposals: [proposal("safe", true), proposal("risk", false), proposal("missing", true, null)] })).toEqual([
      expect.objectContaining({ date: "2031-04-16", amount: 1000, type: "expense", rawPlace: "SAFE RAW" }),
    ]);
  });
});
...
const baseItem = (over: Partial<ApplyItem> = {}): ApplyItem => ({
  ...
  name: "Test",
  tag: "sub",
  rawPlace: "MAPLE HARBOR*MEMBERSHIP",
  envelopeId: ENV,
  categoryId: null,
  placeName: "Maple Harbor",
  ...over,
});
...
describe("import/apply — extended items", () => {
  ...
  it("scopes persisted exact and probable evidence to the item's effective account", () => {
    const result = planImportDryRun({
      ...
      existing: [
        { accountId: ACC_A, toAccountId: null, type: "expense", date: "2031-04-16", amount: 2785, sourceRef: "MAPLE HARBOR*MEMBERSHIP" },
  ...
  it("uses separate within-batch evidence for mixed per-item account overrides", () => {
    const result = planImportDryRun({
      ...
      existing: [],
      items: [
        baseItem({ date: "2031-04-18", rawPlace: "BATCH" }),
        baseItem({ accountId: ACC_B, date: "2031-04-18", rawPlace: "BATCH" }),
        baseItem({ date: "2031-04-18", rawPlace: "BATCH" }),
        baseItem({ accountId: ACC_B, date: "2031-04-18", rawPlace: "BATCH" }),
      ],
    });

...
export function buildImportStatementExtractPrompt(pages: string[], today: string, locale: AiLocale, currency: string): ChatRequest {
  const system =
    ...
    'Opening balance, closing balance, running balances, totals, headings, page numbers, legal footers and account details are ui_metadata rows with amount null. A statement header prints labels on one line and their figures on the next; write each label together with its own figure on ONE rawTextLine (e.g. "CLOSING BALANCE USD 3 750.00", "OPENING BALANCE USD 1 250.00"), keeping every figure. Use null for any unreadable fact; never invent one. Relations: set relation to null unless two rows on these pages visibly belong together. ' +

      ...
      transactions: {
        ...
        items: {
          ...
          properties: {
            ...
            place: { type: ["string", "null"], description: "Readable place name (e.g. Linden Market) — an existing one if it matches" },
          },
...
async function openaiJson(req: ChatRequest): Promise<string> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`OpenAI odpowiedziało błędem ${res.status}. Sprawdź klucz/model (OPENAI_MODEL=${env.OPENAI_MODEL}).`);  
  }
...
importRoutes.post("/import/extract", async (c) => {
  if (!env.OPENAI_API_KEY) {
    return c.json({ error: "Brak klucza OpenAI — ustaw OPENAI_API_KEY w .env i zrestartuj aplikację." }, 503);  
  }
  ...
  const sysEnrich =
    ...
    "place — a readable, short place name (e.g. Linden Market, Netflix). Do not change amounts or dates. " +
...
importRoutes.post("/import/apply", async (c) => {
  ...
  for (const it of body.items) {
    ...
    dupIdx.markSeen({ date: it.date, amount: it.amount, rawPlace: it.rawPlace });  
    if (!body.dryRun) {

...
export function computeBudgetState(ledger: Ledger, month: string): BudgetState {
  ...
  const allocCum = new Map<string, Money>();  
  const allocThis = new Map<string, Money>();  
  ...
  for (const t of txns) {
    ...
    if (t.type === "transfer" && t.toAccountId) {
      ...
      if (toOn && !fromOn) toBeBudgeted += t.amount;  
      if (fromOn && !toOn) toBeBudgeted -= t.amount;  
    }
  }
  ...
  for (const t of allTxns) {
    ...
    if (t.type === "transfer" && t.toAccountId) {
      const fromOn = isOnBudget(t.accountId);
      const toOn = isOnBudget(t.toAccountId);
      if (toOn && !fromOn) readyToAssign += t.amount;  
      if (fromOn && !toOn) readyToAssign -= t.amount;  
    }
  }

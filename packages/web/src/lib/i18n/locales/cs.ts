/** Czech — community translation. Keys are the ENGLISH messages (src/lib/i18n/messages.generated.ts). */
import type { Dict } from "../index";

export const cs: Dict = {
  "+ Add envelope": "+ Přidat obálku",
  "+ Add item": "+ Přidat položku",
  "+ Add profile": "+ Přidat profil",
  "+ Add “{name}”": "+ Přidat „{name}“",
  "+ Group": "+ Skupina",
  "1 mo": "1 měs.",
  "1 yr": "1 rok",
  "12 mo": "12 měs.",
  "3 mo": "3 měs.",
  "6 mo": "6 měs.",
  "= {amount}": "= {amount}",
  "A previous upgrade attempt was interrupted before it could finish. Resume it to complete the upgrade with the password you chose then — a new password cannot be set until this attempt finishes or is discarded.":
    "Předchozí pokus o aktualizaci byl přerušen, než se stihl dokončit. Pokračuj v něm a dokonči aktualizaci tehdy zvoleným heslem — nové heslo nelze nastavit, dokud tento pokus neskončí nebo nebude zahozen.",
  "A profile with this name already exists.": "Profil s tímto názvem už existuje.",
  AI: "AI",
  "AI (corrected)": "AI (opraveno)",
  "AI is off — suggestions run locally on rules; nothing leaves this device.":
    "AI je vypnutá — návrhy počítáme lokálně podle pravidel; toto zařízení nic neopouští.",
  "AI requests go to OpenAI through the app server (operator's key).": "Požadavky AI jdou do OpenAI přes server aplikace (klíč provozovatele).",
  "AI unavailable — rules were used instead.": "AI není dostupná — použila se pravidla.",
  AVAILABLE: "K DISPOZICI",
  Account: "Účet",
  "About {n}× the cost of the cheapest tier.": "Přibližně {n}× cena nejlevnější úrovně.",
  "Account change": "Změna účtu",
  "Account deletion": "Smazání účtu",
  "Account name": "Název účtu",
  Accounts: "Účty",
  "Actual balance (from your bank)": "Skutečný zůstatek (z banky)",
  Add: "Přidat",
  "Add Enveo to your device so it opens like any other app — offline, full screen, one tap away.":
    "Přidej si Enveo do zařízení, ať se otevírá jako každá jiná aplikace — offline, na celou obrazovku, jedním klepnutím.",
  "Add Enveo to your home screen": "Přidat Enveo na plochu",
  "Add Enveo to your home screen so it opens like any other app:": "Přidej si Enveo na plochu, ať se otevírá jako každá jiná aplikace:",
  "Add Enveo to your phone": "Přidat Enveo do telefonu",
  "Add a custom envelope": "Přidat vlastní obálku",
  "Add account": "Přidat účet",
  "Add an account first — you need one to save a transaction.": "Nejdřív přidej účet — bez něj nejde transakci uložit.",
  "Add an envelope to the split": "Přidat obálku do rozdělení",
  "Add expense": "Přidat výdaj",
  "Add income": "Přidat příjem",
  "Add refund": "Přidat vratku",
  "Add screenshots": "Přidat snímky obrazovky",
  "Add the account you spend from. The balance can be approximate — it is easy to adjust later.":
    "Přidej účet, ze kterého utrácíš. Zůstatek může být přibližný — snadno ho později upravíš.",
  "Add to Home Screen": "Přidat na plochu",
  "Add transfer": "Přidat převod",
  "Add your first account": "Přidej svůj první účet",
  "Add {n} transaction | Add {n} transactions": {
    one: "Přidat {n} transakci",
    few: "Přidat {n} transakce",
    many: "Přidat {n} transakce",
    other: "Přidat {n} transakcí",
  },
  "Added {n} transaction | Added {n} transactions": {
    one: "Přidána {n} transakce",
    few: "Přidány {n} transakce",
    many: "Přidáno {n} transakce",
    other: "Přidáno {n} transakcí",
  },
  "Adding…": "Přidávám…",
  "Afterwards, other devices must unlock again with the new password or a fresh pairing code — old pairing codes stop working.":
    "Na ostatních zařízeních pak bude potřeba znovu odemknout novým heslem nebo čerstvým párovacím kódem — staré párovací kódy přestanou fungovat.",
  "All goals funded ✓": "Všechny cíle financovány ✓",
  Allocated: "Přiděleno",
  "Allocated {allocated} · Spent {spent}": "Přiděleno {allocated} · Utraceno {spent}",
  "Allocated: {name}": "Přiděleno: {name}",
  "Already have an account? Sign in": "Už máš účet? Přihlas se",
  "An account with this email already exists — sign in instead.": "Účet s tímto e-mailem už existuje — raději se přihlas.",
  "An entry nothing uses can be deleted for good.": "Položku, kterou nic nepoužívá, lze smazat natrvalo.",
  "An upgrade or a restore is in progress: the account's budget on the server is not this device's budget yet. Once it is, sync will resume by itself.":
    "Probíhá upgrade nebo obnova: rozpočet účtu na serveru zatím není rozpočtem tohoto zařízení. Jakmile bude, synchronizace se sama rozběhne.",
  "Another account's data": "Data jiného účtu",
  "Apple Wallet or bank history — AI will recognize the transactions, duplicates will be skipped":
    "Apple Wallet nebo historie z banky — AI rozpozná transakce, duplicity přeskočíme",
  "Apply ({n})": "Použít ({n})",
  Archive: "Archivovat",
  Archived: "Archivováno",
  "Archived account": "Archivovaný účet",
  "Artificial intelligence": "Umělá inteligence",
  Auto: "Auto",
  Available: "K dispozici",
  BUDGET: "ROZPOČET",
  Back: "Zpět",
  Backup: "Záloha",
  "Backup downloaded.": "Záloha stažena.",
  "Backup loaded — data has been replaced.": "Záloha načtena — data byla nahrazena.",
  Balance: "Zůstatek",
  "Balance adjustment": "Úprava zůstatku",
  "Balance in the app": "Zůstatek v aplikaci",
  "Balance matches": "Zůstatek souhlasí",
  "Balance:": "Zůstatek:",
  Balanced: "Vyvážený",
  "Before you continue, download a JSON backup and keep it somewhere safe.": "Než budeš pokračovat, stáhni si zálohu JSON a ulož ji na bezpečné místo.",
  Bills: "Faktury",
  "Best quality": "Nejvyšší kvalita",
  "Breakdown by category": "Rozpad podle kategorie",
  Budget: "Rozpočet",
  "Budget currency change": "Změna měny",
  "Budget data will be encrypted on your device before it reaches the server. Server-side features will be unavailable.":
    "Data rozpočtu se zašifrují na tvém zařízení, ještě než dorazí na server. Serverové funkce nebudou dostupné.",
  Budgets: "Rozpočty",
  "By envelope": "Podle obálky",
  "By history": "Podle historie",
  "By place": "Podle místa",
  "CARRIED OVER": "PŘEVEDENO",
  Cancel: "Zrušit",
  "Cancel editing": "Zrušit úpravu",
  "Cash flow": "Cashflow",
  "Categories and places": "Kategorie a místa",
  Category: "Kategorie",
  "Category change": "Změna kategorie",
  "Category deletion": "Smazání kategorie",
  Change: "Změnit",
  "Change encryption password": "Změnit heslo šifrování",
  "Change password": "Změnit heslo",
  "Changing…": "Měním…",
  "Check again": "Zkontrolovat znovu",
  "Checking which models your key can use…": "Zjišťuje se, které modely tvůj klíč podporuje…",
  "Checking…": "Kontroluji…",
  "Choose a color": "Vyber barvu",
  "Choose an account": "Vyber účet",
  "Choose an envelope": "Vyber obálku",
  "Choose an icon": "Vyber ikonu",
  Cisza: "Ticho",
  "Clear & reset": "Vyprázdnit a resetovat",
  "Clear search": "Vymazat hledání",
  Close: "Zavřít",
  "Close details": "Zavřít detaily",
  Closed: "Zavřené",
  Collapse: "Sbalit",
  Color: "Barva",
  "Color {color}": "Barva {color}",
  "Community translation — it may be incomplete.": "Komunitní překlad — může být neúplný.",
  "Copied.": "Zkopírováno.",
  "Copy code": "Zkopírovat kód",
  "Could not check model availability right now — every tier stays selectable.":
    "Dostupnost modelů se teď nepodařilo ověřit — všechny úrovně zůstávají na výběr.",
  "Could not create the account — please try again.": "Účet se nepodařilo vytvořit — zkus to znovu.",
  "Could not reach the AI service — check the network connection and try again.":
    "Službu AI se nepodařilo kontaktovat — zkontroluj připojení k síti a zkus to znovu.",
  "Could not sign in — please try again.": "Přihlášení se nezdařilo — zkus to znovu.",
  "Create account": "Vytvořit účet",
  "Create an account": "Vytvoř si účet",
  "Create envelopes": "Vytvořit obálky",
  "Create the owner account": "Vytvořit účet vlastníka",
  "Create your first envelopes": "Vytvoř si první obálky",
  Currency: "Měna",
  "Current password": "Současné heslo",
  Custom: "Vlastní",
  "Custom color": "Vlastní barva",
  "Custom envelope…": "Vlastní obálka…",
  "Custom profiles are stored only on this device.": "Vlastní profily jsou uložené jen na tomto zařízení.",
  "DISABLE-E2EE": "VYPNOUT-E2EE",
  Dark: "Tmavý",
  Delete: "Smazat",
  "Delete everything and start fresh": "Smazat vše a začít od nuly",
  "Delete everything and start over": "Smazat vše a začít znovu",
  "Delete group": "Smazat skupinu",
  "Delete profile “{name}”?": "Smazat profil „{name}“?",
  "Delete the empty group “{name}”?": "Smazat prázdnou skupinu „{name}“?",
  "Delete the envelope “{name}”? Its transactions will be left without an envelope.": "Smazat obálku „{name}“? Její transakce zůstanou bez obálky.",
  "Delete this transaction? This cannot be undone.": "Smazat tuto transakci? Tuto akci nelze vrátit zpět.",
  "Delete {name}": "Smazat {name}",
  "Delete “{name}” for good? It is not used by any transaction.": "Smazat „{name}“ natrvalo? Nepoužívá ji žádná transakce.",
  "Deletes ALL budget data from the server and this device — irreversible. Export a backup first. The app will reopen with the first-run wizard.":
    "Smaže VŠECHNA data rozpočtu ze serveru i z tohoto zařízení — nevratně. Nejdřív si vyexportuj zálohu. Aplikace se znovu otevře s úvodním průvodcem.",
  "Deleting…": "Mažu…",
  "Describe how to distribute (e.g. prioritize savings, less on entertainment)": "Popiš, jak rozdělit (např. upřednostnit spoření, méně na zábavu)",
  "Destination account": "Cílový účet",
  "Difference: {sign}{amount} → this will create a correcting {kind}": "Rozdíl: {sign}{amount} → vznikne opravný {kind}",
  "Disable encryption": "Vypnout šifrování",
  "Disable end-to-end encryption": "Vypnout end-to-end šifrování",
  "Disable end-to-end encryption?": "Vypnout end-to-end šifrování?",
  "Disabling…": "Vypínám…",
  Discard: "Zahodit",
  "Discard the unfinished attempt": "Zahodit nedokončený pokus",
  "Discarding starts over with a fresh key. If the server had already completed the interrupted attempt, this device will ask you to unlock with the password you chose then.":
    "Zahozením se začne znovu s čerstvým klíčem. Pokud server přerušený pokus už stihl dokončit, toto zařízení tě požádá o odemknutí tehdy zvoleným heslem.",
  "Discreet mode": "Diskrétní režim",
  "Distributes the amount only to envelopes in the red — proportionally to shortfalls, never past zero.":
    "Rozdělí částku jen mezi obálky v mínusu — poměrně podle schodku, nikdy přes nulu.",
  Done: "Hotovo",
  "Download a backup (JSON)": "Stáhnout zálohu (JSON)",
  "Drag {name}": "Přetáhnout {name}",
  Duplicate: "Duplikovat",
  "Duplicates skipped: {n}": "Přeskočené duplicity: {n}",
  E2EE: "E2EE",
  Edit: "Upravit",
  Economical: "Úsporný",
  "Edit account": "Upravit účet",
  "Edit envelope": "Upravit obálku",
  "Edit item {n}": "Upravit položku {n}",
  "Edit names in place; drag the handle to reorder": "Názvy uprav přímo na místě; pořadí změníš tažením za úchyt",
  Email: "E-mail",
  "Enable AI assistance?": "Zapnout asistenci AI?",
  "Enable end-to-end encryption": "Zapnout end-to-end šifrování",
  "Enable via server": "Zapnout přes server",
  "Enabled — the server stores only encrypted data and never knows your password or key.":
    "Zapnuto — server ukládá jen zašifrovaná data a nikdy nezná tvé heslo ani klíč.",
  "Enabling failed — nothing was changed, your data stays as it was.": "Zapnutí selhalo — nic se nezměnilo, tvá data zůstávají, jak byla.",
  "Encrypt and enable": "Zašifrovat a zapnout",
  "Encrypting…": "Šifruji…",
  "Encryption password": "Heslo šifrování",
  "Encryption password (min. 10 characters)": "Heslo šifrování (min. 10 znaků)",
  Envelope: "Obálka",
  "Envelope allocation": "Přidělení do obálky",
  "Envelope budgeting: assign your income to envelopes and always know how much you can still spend.":
    "Obálkové rozpočtování: rozděl příjmy do obálek a vždy víš, kolik ještě můžeš utratit.",
  "Envelope budgets": "Rozpočty obálek",
  "Envelope change": "Změna obálky",
  "Envelope deletion": "Smazání obálky",
  "Envelope group change": "Změna skupiny obálek",
  "Envelope group deletion": "Smazání skupiny obálek",
  "Envelope name": "Název obálky",
  Envelopes: "Obálky",
  "Envelopes have changed since this was generated — generate a new suggestion.": "Obálky se od vygenerování změnily — vygeneruj nový návrh.",
  "Envelopes stay as they are — you assigned this by hand.": "Obálky zůstanou beze změny — rozdělil jsi to ručně.",
  "Excess {amount}": "Přebývá {amount}",
  "Exclude from the total": "Vyloučit ze součtu",
  "Include again": "Znovu zahrnout",
  Reset: "Obnovit",
  "Without {names}": "Bez: {names}",
  Expense: "Výdaj",
  "Export backup (JSON)": "Exportovat zálohu (JSON)",
  "Failed to load the image.": "Obrázek se nepodařilo načíst.",
  Filter: "Filtr",
  "Filter:": "Filtr:",
  "Finance & shopping": "Finance a nákupy",
  "Food & drink": "Jídlo a pití",
  "From account": "Z účtu",
  "From screenshot": "Ze snímku obrazovky",
  Fun: "Zábava",
  "Generate suggestion": "Vygenerovat návrh",
  "Generating…": "Generuji…",
  Goals: "Cíle",
  "Goes to": "Půjde do",
  Good: "Dobré",
  "Google sign-in keeps you signed in until you sign out — remember to sign out when you finish.":
    "Přihlášení přes Google tě drží přihlášeného, dokud se neodhlásíš — nezapomeň se po skončení odhlásit.",
  Groceries: "Potraviny",
  Group: "Skupina",
  Health: "Zdraví",
  "Health & fitness": "Zdraví a kondice",
  Hidden: "Skryté",
  Hide: "Skrýt",
  "Hiding an entry only removes it from suggestions. Transactions that use it keep it, here and in reports.":
    "Skrytí položky ji odebere jen z našeptávání. Transakce, které ji používají, si ji ponechají — tady i v přehledech.",
  Home: "Domů",
  "Home & bills": "Bydlení a účty",
  Housing: "Bydlení",
  "I have a backup in a safe place": "Mám zálohu na bezpečném místě",
  "I have a pairing code": "Mám párovací kód",
  Icon: "Ikona",
  "If this is your data, sign in with the previous account — the copy is intact. After restoring a server from a backup the account id can be new even though the e-mail is the same.":
    "Pokud jsou to tvá data, přihlas se předchozím účtem — kopie je v pořádku. Po obnově serveru ze zálohy může být id účtu nové, i když e-mail je stejný.",
  "If “Last launch: fetched from server” appears every time you open the app, iOS is deleting the local copy between sessions — that is why the first load is slow. “Persistent storage: Yes” lowers the risk of such eviction.":
    "Pokud se při každém otevření aplikace objeví „Poslední spuštění: staženo ze serveru“, iOS mezi relacemi maže lokální kopii — proto je první načtení pomalé. „Trvalé úložiště: Ano“ riziko takového vyhození snižuje.",
  "Import from screenshots": "Import ze snímků obrazovky",
  "Imported item": "Importovaná položka",
  Income: "Příjem",
  Install: "Nainstalovat",
  "Install app": "Nainstalovat aplikaci",
  "Invalid pairing code.": "Neplatný párovací kód.",
  Investor: "Investor",
  "Item total: {sum} / {total}": "Součet položek: {sum} / {total}",
  Language: "Jazyk",
  "Last launch": "Poslední spuštění",
  "Last sync: {rel}.": "Poslední synchronizace: {rel}.",
  "Left after changes": "Zbude po změnách",
  "Leisure & learning": "Volný čas a vzdělávání",
  Light: "Světlý",
  "Like last month": "Jako minulý měsíc",
  Living: "Běžný život",
  "Load backup (JSON)": "Načíst zálohu (JSON)",
  "Loading sample data…": "Načítám ukázková data…",
  "Loading…": "Načítám…",
  "Losing the password means losing your data — the server cannot reset it or decrypt your budget.":
    "Ztratit heslo znamená ztratit data — server ho neumí resetovat ani tvůj rozpočet dešifrovat.",
  "Lowest cost — a typical screenshot import costs a fraction of a cent.": "Nejnižší cena — typický import snímku obrazovky stojí zlomek centu.",
  "Manage envelopes": "Spravovat obálky",
  "Manage…": "Spravovat…",
  "Median of historical monthly spending — resistant to one-off spikes; the free remainder is spread proportionally.":
    "Medián historických měsíčních výdajů — odolný vůči jednorázovým výkyvům; volný zbytek se rozdělí poměrně.",
  Menu: "Menu",
  Mode: "Režim",
  Model: "Model",
  "Monthly breakdown": "Rozpad po měsících",
  "Monthly target (optional)": "Měsíční cíl (nepovinné)",
  More: "Více",
  "More colors": "Více barev",
  "Move the money between envelopes too": "Přesunout peníze i mezi obálkami",
  "NO AI": "BEZ AI",
  Name: "Název",
  Net: "Netto",
  "Net worth": "Čistý majetek",
  "Net worth over time": "Vývoj čistého majetku",
  "New account": "Nový účet",
  "New category": "Nová kategorie",
  "New envelope": "Nová obálka",
  "New envelope group": "Nová skupina obálek",
  "New group": "Nová skupina",
  "New place": "Nové místo",
  "New transaction": "Nová transakce",
  "New version available": "Je dostupná nová verze",
  Next: "Dále",
  "Next month": "Další měsíc",
  No: "Ne",
  "No account yet? Create one": "Ještě nemáš účet? Vytvoř si ho",
  "No envelopes are marked as wealth envelopes. Open an envelope → Edit and turn on “Wealth envelope” (e.g. Bonds, Retirement, Savings), and we will count them here.":
    "Žádná obálka není označená jako majetková. Otevři obálku → Upravit a zapni „Majetková obálka“ (např. Dluhopisy, Důchod, Spoření) a započítáme ji sem.",
  "No envelopes with a budget or spending this month.": "Žádné obálky s rozpočtem nebo výdaji v tomto měsíci.",
  "No envelopes with a goal. Set a monthly target when editing an envelope.": "Žádné obálky s cílem. Měsíční cíl nastav při úpravě obálky.",
  "No funds to distribute": "Nejsou žádné prostředky k rozdělení",
  "No spending in this period.": "V tomto období žádné výdaje.",
  "No transactions.": "Žádné transakce.",
  "Not available with your OpenAI key.": "Není dostupný s tvým klíčem OpenAI.",
  "Not sending": "Neodesílá se",
  "Not syncing with your account": "Nesynchronizuje se s tvým účtem",
  "Nothing here yet.": "Zatím tu nic není.",
  Off: "Vypnuto",
  "Once enabled, the server stores ciphertexts only. The key is your password, which the server does NOT know — without it (or a pairing code from another unlocked device) the data cannot be recovered.":
    "Po zapnutí server ukládá jen šifrované texty. Klíčem je tvé heslo, které server NEZNÁ — bez něj (nebo bez párovacího kódu z důvěryhodného zařízení) data nelze obnovit.",
  "One tap and Enveo lives on your home screen — offline, full screen, no browser bar.":
    "Jedno klepnutí a Enveo je na tvé ploše — offline, na celou obrazovku, bez lišty prohlížeče.",
  "Open {host} in Safari to add it to your home screen — installing only works from Safari on iPhone and iPad.":
    "Otevři {host} v Safari a přidej aplikaci na plochu — na iPhonu a iPadu instalace funguje jen ze Safari.",
  "OpenAI rejected the request — check the key and the model, then try again.": "OpenAI požadavek odmítlo — zkontroluj klíč a model a zkus to znovu.",
  "OpenAI rejected this key — model availability could not be checked.": "OpenAI tento klíč odmítlo — dostupnost modelů se nepodařilo ověřit.",
  "OpenAI rejected your key — check it in Settings → Artificial intelligence.": "OpenAI odmítlo tvůj klíč — zkontroluj ho v Nastavení → Umělá inteligence.",
  Other: "Ostatní",
  "Other colors": "Další barvy",
  "Own key": "Vlastní klíč",
  "Pairing code": "Párovací kód",
  "Pairing code unavailable — no key on this device.": "Párovací kód není dostupný — na tomto zařízení není klíč.",
  "Pairing code unavailable — this device's key has not been confirmed for the budget's current encryption yet. Unlock again or let a sync finish first.":
    "Párovací kód není dostupný — klíč tohoto zařízení ještě nebyl potvrzen pro aktuální šifrování rozpočtu. Odemkni znovu, nebo nech nejdřív doběhnout synchronizaci.",
  Palette: "Paleta",
  Password: "Heslo",
  "Password changed.": "Heslo změněno.",
  "Passwords do not match.": "Hesla se neshodují.",
  "Paste the pairing code shown on another unlocked device (Settings → Pairing code).":
    "Vlož párovací kód zobrazený na důvěryhodném zařízení (Nastavení → Párovací kód).",
  "People & pets": "Lidé a mazlíčci",
  "Persistent storage": "Trvalé úložiště",
  "Pick the envelopes you want to start with — you can change them or add new ones anytime.":
    "Vyber obálky, se kterými chceš začít — kdykoli je můžeš změnit nebo přidat nové.",
  Place: "Místo",
  "Place change": "Změna místa",
  "Place deletion": "Smazání místa",
  Predefined: "Přednastavené",
  "Previous month": "Předchozí měsíc",
  Privacy: "Soukromí",
  "Previously selected model — it stays available until you pick a tier.": "Dříve zvolený model — zůstává k dispozici, dokud nezvolíš úroveň.",
  "Process screenshots": "Zpracovat snímky obrazovky",
  Prompt: "Zadání",
  RESET: "RESET",
  "Rainy day": "Železná rezerva",
  "Re-encrypt and upgrade": "Znovu zašifrovat a aktualizovat",
  "Recognizing…": "Rozpoznávám…",
  Reconcile: "Spárovat",
  "Reconcile account": "Spárovat účet",
  Refresh: "Obnovit",
  "Registration is closed on this server.": "Registrace je na tomto serveru uzavřená.",
  "Remaining {amount}": "Zbývá {amount}",
  Remove: "Odebrat",
  "Remove screenshot {n}": "Odebrat snímek obrazovky {n}",
  "Remove this data and continue": "Odebrat tato data a pokračovat",
  "Removing…": "Odebírám…",
  "Repeat password": "Zopakuj heslo",
  "Replaces all current data with the data from the selected file.": "Nahradí všechna současná data daty z vybraného souboru.",
  "Report a fix": "Nahlásit opravu",
  Reports: "Přehledy",
  "Requires AI (server mode or your own key).": "Vyžaduje AI (serverový režim nebo vlastní klíč).",
  Restore: "Obnovit",
  "Restore “{name}” · hidden": "Obnovit „{name}“ · skryté",
  "Resume upgrade": "Pokračovat v aktualizaci",
  SPENT: "UTRACENO",
  Save: "Uložit",
  "Save changes": "Uložit změny",
  "Save item": "Uložit položku",
  Savings: "Spoření",
  "Savings envelopes first — the free remainder goes to them.": "Nejdřív spořicí obálky — volný zbytek půjde do nich.",
  "Screenshot {n}": "Snímek obrazovky {n}",
  "Screenshots ({n}/6)": "Snímky obrazovky ({n}/6)",
  "Search...": "Hledat...",
  Server: "Server",
  "Server sync": "Synchronizace se serverem",
  "Server temporarily unreachable — your data is safe, we will retry.": "Server je dočasně nedostupný — tvá data jsou v bezpečí, zkusíme to znovu.",
  "Session expired": "Relace vypršela",
  Settings: "Nastavení",
  Share: "Sdílet",
  "Short after changes": "Chybí po změnách",
  "Sign in": "Přihlásit se",
  "Sign in to Enveo": "Přihlaš se do Enveo",
  "Sign in with Google": "Přihlásit se přes Google",
  "Sign out": "Odhlásit se",
  "Sign out and use another account": "Odhlásit se a použít jiný účet",
  "Skip for now": "Zatím přeskočit",
  "Some funds stayed in To be budgeted — envelopes are at their target caps.": "Část prostředků zůstala v „K rozdělení“ — obálky dosáhly stropu svého cíle.",
  Sort: "\u0158adit",
  "Source: {src}": "Zdroj: {src}",
  Spending: "Výdaje",
  "Split across envelopes": "Rozdělit mezi obálky",
  "Split transaction": "Rozdělit transakci",
  "Start from scratch": "Začít od nuly",
  "Start with an empty budget": "Začít s prázdným rozpočtem",
  "Starting balance": "Počáteční zůstatek",
  "Starting balance (0)": "Počáteční zůstatek (0)",
  "Stick with rules": "Zůstat u pravidel",
  Strategy: "Strategie",
  Strong: "Silné",
  Subscriptions: "Předplatné",
  Suggest: "Navrhnout",
  "Suggest a distribution": "Navrhnout rozdělení",
  Summary: "Souhrn",
  "Sync in progress": "Probíhá synchronizace",
  "Sync now": "Synchronizovat teď",
  "Syncing…": "Synchronizuji…",
  "Tap {action} to continue": "Klepni na {action} a pokračuj",
  "That does not look like a valid email address.": "Tohle nevypadá jako platná e-mailová adresa.",
  "That password is too long.": "Toto heslo je příliš dlouhé.",
  "The AI model (OpenAI) will receive: the amount to distribute, envelope names and aggregated spending stats from recent months.":
    "Model AI (OpenAI) dostane: částku k rozdělení, názvy obálek a souhrnné statistiky výdajů za poslední měsíce.",
  "The AI model (OpenAI) will receive: the screenshots and the names of your envelopes and categories.":
    "Model AI (OpenAI) dostane: snímky obrazovky a názvy tvých obálek a kategorií.",
  "The AI service took too long to answer — nothing was changed. Try again in a moment.":
    "Služba AI odpovídala příliš dlouho — nic se nezměnilo. Zkus to za chvíli znovu.",
  "The account “{name}” will disappear from the Start screen and lists (you will find it under “Closed” on the Accounts screen). Its transactions and balance still count in the budget and reports.\n\nArchive it?":
    "Účet „{name}“ zmizí z úvodní obrazovky i ze seznamů (najdeš ho pod „Zavřené“ na obrazovce Účty). Jeho transakce a zůstatek se dál počítají do rozpočtu a přehledů.\n\nArchivovat?",
  "The agent proposed no distribution — refine your prompt.": "Agent nenavrhl žádné rozdělení — upřesni zadání.",
  "The app talks to OpenAI directly from this browser using your own key — bypassing the server.":
    "Aplikace mluví s OpenAI přímo z tohoto prohlížeče tvým vlastním klíčem — server obchází.",
  "The backup is corrupted and was not loaded: {detail}": "Záloha je poškozená a nebyla načtena: {detail}",
  "The budget's encryption was already upgraded on another device — unlock with the new encryption password.":
    "Šifrování rozpočtu už bylo aktualizováno na jiném zařízení — odemkni rozpočet novým heslem šifrování.",
  "The budget changed on the server in the meantime — nothing was written. Try again.":
    "Rozpočet se mezitím na serveru změnil — nic se nezapsalo. Zkus to znovu.",
  "The budget has changed since this was generated — generate a new suggestion.": "Rozpočet se od vygenerování změnil — vygeneruj nový návrh.",
  "The budget data on this device could not be matched to the account you are signed in with, so nothing can be sent to the server. Nothing has been deleted yet. Download a backup first — this device may hold the only copy of that data. “Delete everything and start fresh” erases this account's data on the server and the local copy on this device, then signs you out.":
    "Data rozpočtu na tomto zařízení se nepodařilo přiřadit k přihlášenému účtu, takže na server nelze nic odeslat. Zatím se nic nesmazalo. Nejdřív si stáhni zálohu — toto zařízení může mít jedinou kopii těchto dat. „Smazat vše a začít od nuly“ smaže data tohoto účtu na serveru i lokální kopii na tomto zařízení a pak tě odhlásí.",
  "The budget is not empty — demo data can only be loaded into an empty budget.": "Rozpočet není prázdný — ukázková data lze načíst jen do prázdného rozpočtu.",
  "The check repeats by itself when you open the app, and roughly once a minute.": "Kontrola se opakuje sama při otevření aplikace a zhruba jednou za minutu.",
  "The copy of the budget on this device has not been matched to the account you are signed in with. Nothing is being sent to the server and nothing has been deleted — your changes are waiting safely here. There are two reasons this happens:":
    "Kopie rozpočtu na tomto zařízení nebyla přiřazena k účtu, kterým ses přihlásil. Na server se nic neposílá a nic se nesmazalo — tvé změny tu bezpečně čekají. Důvody jsou dva:",
  "The data references records that do not exist here (a corrupted or foreign file). Nothing was changed.":
    "Data odkazují na záznamy, které tu neexistují (poškozený nebo cizí soubor). Nic se nezměnilo.",
  "The encrypted data could not be read on this device — nothing was changed. Make sure the app is up to date, or restore from a backup.":
    "Zašifrovaná data se na tomto zařízení nepodařilo přečíst — nic se nezměnilo. Ověř, že máš aktuální verzi aplikace, nebo obnov data ze zálohy.",
  "The envelope “{name}” will disappear from the Budget and Start screens. Its transaction history stays, and available funds remain in the envelope. You can restore it in “Manage envelopes” → Archived.\n\nArchive it?":
    "Obálka „{name}“ zmizí z obrazovek Rozpočet a Domů. Historie transakcí zůstane a dostupné prostředky zůstanou v obálce. Obnovit ji můžeš ve „Spravovat obálky“ → Archivované.\n\nArchivovat?",
  "The first launch requires a connection to the server": "První spuštění vyžaduje připojení k serveru",
  "The local copy of the budget has not loaded yet — nothing was sent. Reload the app and try again.":
    "Lokální kopie rozpočtu se ještě nenačetla — nic se neodeslalo. Načti aplikaci znovu a zkus to znovu.",
  "The local copy of the budget on this device belongs to a different account than the one you are signed in with. Nothing has been sent to the server and nothing has been deleted.":
    "Lokální kopie rozpočtu na tomto zařízení patří jinému účtu, než kterým ses přihlásil. Na server se nic neodeslalo a nic se nesmazalo.",
  "The local copy — including any unsent changes — will be permanently removed from this device. If this is the only copy of that budget, download a backup first.":
    "Lokální kopie — včetně neodeslaných změn — se z tohoto zařízení trvale odstraní. Pokud je to jediná kopie toho rozpočtu, nejdřív si stáhni zálohu.",
  "The local copy — including any unsent changes — will be permanently removed from this device. Your account's data on the server is not affected.":
    "Lokální kopie — včetně neodeslaných změn — se z tohoto zařízení trvale odstraní. Údaje tvého účtu na serveru zůstanou beze změny.",
  "The local replica is not ready.": "Lokální replika není připravená.",
  "The password must be at least 8 characters.": "Heslo musí mít aspoň 8 znaků.",
  "The proposal exceeds “To be budgeted” — uncheck or reduce items.": "Návrh překračuje „K rozdělení“ — odškrtni nebo sniž položky.",
  "The server hit an unexpected error. Nothing was changed — try again.": "Na serveru nastala neočekávaná chyba. Nic se nezměnilo — zkus to znovu.",
  "The server refuses to sync this budget until its encryption is upgraded to the new format.":
    "Server odmítá tento rozpočet synchronizovat, dokud se jeho šifrování neaktualizuje na nový formát.",
  "The server rejected these changes — usually because you edited something that was meanwhile deleted on another device. Your data has already been restored to the server state. “Discard” removes the failed attempt from the list.":
    "Server tyto změny odmítl — obvykle proto, že jsi upravil něco, co mezitím na jiném zařízení někdo smazal. Tvá data jsme už vrátili do stavu ze serveru. „Zahodit“ odstraní neúspěšný pokus ze seznamu.",
  "The server rejected {n} change — tap to open settings | The server rejected {n} changes — tap to open settings": {
    one: "Server odmítl {n} změnu — klepnutím otevřeš nastavení",
    few: "Server odmítl {n} změny — klepnutím otevřeš nastavení",
    many: "Server odmítl {n} změny — klepnutím otevřeš nastavení",
    other: "Server odmítl {n} změn — klepnutím otevřeš nastavení",
  },
  "The signed-in account changed while the data was being sent — nothing was written. Reload the app and try again.":
    "Přihlášený účet se během odesílání dat změnil — nic se nezapsalo. Načti aplikaci znovu a zkus to znovu.",
  "The upgrade failed — nothing was changed on the server.": "Aktualizace se nezdařila — na serveru se nic nezměnilo.",
  "The upload is too large — try fewer (or smaller) screenshots.": "Odesílaná data jsou příliš velká — zkus méně (nebo menší) snímků obrazovky.",
  Theme: "Motiv",
  "Then tap {action}": "Potom klepni na {action}",
  "There is no data on this device to send — nothing was sent to the server. Reload the app to fetch your budget first.":
    "Na tomto zařízení nejsou žádná data k odeslání — na server se nic neodeslalo. Načti aplikaci znovu, aby se nejdřív stáhl tvůj rozpočet.",
  "There is nothing to export yet — wait for the app to finish loading.": "Zatím není co exportovat — počkej, až se aplikace donačte.",
  "This budget is encrypted": "Tento rozpočet je zašifrovaný",
  "This budget is encrypted with an older format that new versions of the app no longer read. Upgrading re-encrypts it with a fresh key built from the data on THIS device: pick a new encryption password (it may be the same one), and the server's copy is replaced in one step.":
    "Tento rozpočet je zašifrovaný starším formátem, který nové verze aplikace už nečtou. Aktualizace ho znovu zašifruje čerstvým klíčem vytvořeným z dat na TOMTO zařízení: zvol nové heslo šifrování (může být stejné) a kopie na serveru se nahradí v jednom kroku.",
  "This budget needs an encryption upgrade": "Tento rozpočet potřebuje aktualizaci šifrování",
  "This budget was encrypted with an older format that this version of the app no longer reads, and this device has no copy of the data. Open Enveo on the device that holds the budget (or restore a JSON backup there) and run the encryption upgrade in Settings → Privacy — then unlock here with the new password.":
    "Tento rozpočet byl zašifrovaný starším formátem, který tato verze aplikace už nepřečte, a toto zařízení nemá kopii dat. Otevři Enveo na zařízení, které rozpočet má (nebo tam obnov zálohu JSON), a spusť aktualizaci šifrování v Nastavení → Soukromí — pak tady odemkni novým heslem.",
  "This budget's data is end-to-end encrypted. Enter the encryption password to unlock it on this device.":
    "Data tohoto rozpočtu jsou šifrovaná end-to-end. Zadej heslo šifrování, abys je na tomto zařízení odemkl.",
  "This budget's encryption must be upgraded before it can sync — open Settings → Privacy on a device that holds the data and run the upgrade.":
    "Šifrování tohoto rozpočtu je potřeba aktualizovat, než se bude moct synchronizovat — otevři Nastavení → Soukromí na zařízení, které data má, a spusť aktualizaci.",
  "This code contains your encryption key in plain form. Show it only on your own private device — anyone with the code can read the budget.":
    "Tento kód obsahuje tvůj šifrovací klíč v čitelné podobě. Ukazuj ho jen na svém důvěryhodném zařízení — kdokoli s tímto kódem si může rozpočet přečíst.",
  "This data may belong to a different account. Then it will never be sent — download a backup and remove the copy from this device.":
    "Tato data možná patří jinému účtu. Pak se nikdy neodešlou — stáhni si zálohu a kopii z tohoto zařízení odstraň.",
  "This data uses an older encryption format that this version no longer reads — run the encryption upgrade in Settings → Privacy on the device that holds the budget.":
    "Tato data používají starší formát šifrování, který tato verze už nepřečte — spusť aktualizaci šifrování v Nastavení → Soukromí na zařízení, které rozpočet má.",
  "This device has no encryption key — unlock the budget with your password (or a pairing code) and try again.":
    "Toto zařízení nemá šifrovací klíč — odemkni rozpočet heslem (nebo párovacím kódem) a zkus to znovu.",
  "This device's data has not been matched to your account — nothing is being sent to the server. Tap to open settings":
    "Data tohoto zařízení nebyla přiřazena k tvému účtu — na server se nic neposílá. Klepnutím otevřeš nastavení",
  "This device's local copy cannot be linked to this account": "Lokální kopii tohoto zařízení nelze propojit s tímto účtem",
  "This device's local copy could not be confirmed to belong to the signed-in account — nothing was sent to the server. Settings → Sync explains what happened and what you can do.":
    "Nepodařilo se potvrdit, že lokální kopie tohoto zařízení patří přihlášenému účtu — na server se nic neodeslalo. Nastavení → Synchronizace vysvětlí, co se stalo a co s tím.",
  "This is my private device": "Tohle je moje soukromé zařízení",
  "This is not a backup of this app — choose a file exported from this application.":
    "Tohle není záloha této aplikace — vyber soubor vyexportovaný z této aplikace.",
  "This is not a valid JSON file.": "Tohle není platný soubor JSON.",
  "This is not a valid backup file — nothing was loaded.": "Tohle není platný soubor se zálohou — nic se nenačetlo.",
  "This is not a valid pairing code — copy it again from the device where the budget is already unlocked.":
    "Tohle není platný párovací kód — zkopíruj ho znovu ze zařízení, kde je rozpočet už odemčený.",
  "This is the first account on this server — once it exists, registration closes.":
    "Tohle je první účet na tomto serveru — jakmile bude existovat, registrace se uzavře.",
  "This pairing code belongs to a different budget.": "Tento párovací kód patří jinému rozpočtu.",
  "This part of the app could not be loaded.": "Tuto část aplikace se nepodařilo načíst.",
  "This pairing code is no longer valid — the budget was re-encrypted with a new key after the code was created. Generate a fresh code on a device that is already unlocked.":
    "Tento párovací kód už neplatí — rozpočet byl po vytvoření kódu znovu zašifrován novým klíčem. Vygeneruj čerstvý kód na zařízení, které je už odemčené.",
  "This will replace all current data. Continue?": "Tímto nahradíš všechna současná data. Pokračovat?",
  "To account": "Na účet",
  "To be budgeted": "K rozdělení",
  "To be budgeted is ≤ 0 — there is nothing to distribute.": "„K rozdělení“ je ≤ 0 — není co rozdělovat.",
  "To be budgeted:": "K rozdělení:",
  Today: "Dnes",
  "Toggle refund": "Přepnout vratku",
  "Too short (min. 10 characters)": "Příliš krátké (min. 10 znaků)",
  "Top up negatives": "Dorovnat mínusy",
  "Tops envelopes up to last month's allocations. Uncheck what you don't want.": "Doplní obálky na přidělení z minulého měsíce. Odškrtni, co nechceš.",
  Total: "Celkem",
  "Total spending": "Celkové výdaje",
  Transaction: "Transakce",
  "Transaction change": "Změna transakce",
  "Transaction deletion": "Smazání transakce",
  Transactions: "Transakce",
  Transfer: "Převod",
  Transport: "Doprava",
  "Try again": "Zkusit znovu",
  "Try it with sample data": "Vyzkoušet na ukázkových datech",
  "Type or pick a category...": "Napiš nebo vyber kategorii...",
  "Type or pick a place...": "Napiš nebo vyber místo...",
  "Type {word} to confirm:": "Pro potvrzení napiš {word}:",
  Unlock: "Odemknout",
  "Unlock with password": "Odemknout heslem",
  "Unlocking…": "Odemykám…",
  "Upgrade encryption": "Aktualizovat šifrování",
  Usage: "Využití",
  Use: "Použít",
  Uses: "Pou\u017eit\u00ed",
  Utilities: "Energie a služby",
  Visible: "Viditeln\u00e9",
  "WARNING: IndexedDB unavailable — data is kept only in session memory (it will not survive closing the app).":
    "VAROVÁNÍ: IndexedDB není dostupná — data zůstávají jen v paměti relace (zavření aplikace nepřežijí).",
  "We will send them once the server is reachable.": "Odešleme je, jakmile bude server dostupný.",
  Weak: "Slabé",
  Wealth: "Majetek",
  "Wealth envelope (savings/investments)": "Majetková obálka (spoření/investice)",
  "Welcome to Enveo": "Vítej v Enveo",
  "What Enveo suggests while you add a transaction": "Co Enveo našeptává při přidávání transakce",
  "Wrong email or password.": "Špatný e-mail nebo heslo.",
  "Wrong encryption password.": "Špatné heslo šifrování.",
  Yes: "Ano",
  "Yes, remove the data from this device": "Ano, odebrat data z tohoto zařízení",
  Yesterday: "Včera",
  "You are offline — screenshot import needs a connection. Manual entry works without one.":
    "Jsi offline — import ze snímků potřebuje připojení. Ruční zadání funguje i bez něj.",
  "Your budget is tied to your account. Sign in to continue.": "Tvůj rozpočet je vázaný na tvůj účet. Pro pokračování se přihlas.",
  "Your data will be decrypted and stored on the server in plain form (as before enabling). Make sure you have a current backup.":
    "Tvá data se dešifrují a uloží na server v čitelné podobě (jako před zapnutím). Ověř si, že máš aktuální zálohu.",
  "Your envelopes": "Tvé obálky",
  "Your first account": "Tvůj první účet",
  "assign the rest": "přiřadit zbytek",
  "available after: {amount}": "poté k dispozici: {amount}",
  "back in the envelope": "vrátí se do obálky",
  "balance after the deposit": "zůstatek po vkladu",
  "balance after the refund": "zůstatek po vrácení",
  "cancel split": "zrušit rozdělení",
  dark: "tmavý",
  details: "detaily",
  discreet: "diskrétní",
  "e.g. 5000": "např. 5000",
  "e.g. Checking": "např. Běžný účet",
  "e.g. paycheck, invoice": "např. výplata, faktura",
  "e.g. refund for shoes": "např. vrácení za boty",
  "e.g. transfer to savings": "např. převod do úspor",
  "e.g. weekly groceries": "např. týdenní nákup",
  edited: "upraveno",
  expense: "výdaj",
  "fetched from server": "staženo ze serveru",
  "from local copy": "z lokální kopie",
  "funded ✓": "financováno ✓",
  income: "příjem",
  "just now": "právě teď",
  "left after this": "zůstane po operaci",
  "left in the envelope": "zůstane v obálce",
  "m/m": "m/m",
  "new password for all devices": "nové heslo pro všechna zařízení",
  "not used yet": "zatím nepoužito",
  "not yet": "zatím ne",
  off: "vypnuto",
  "optional · at the end": "nepovinné · na konec",
  "over by {amount}": "překročeno o {amount}",
  "over the envelope": "překročíš obálku",
  "own key": "vlastní klíč",
  "range {min}–{max}": "rozsah {min}–{max}",
  "re-runs the check against the server; nothing is sent": "znovu spustí kontrolu proti serveru; nic se neodesílá",
  "removes the local copy and downloads your account's data": "odebere lokální kopii a stáhne data tvého účtu",
  rules: "pravidla",
  "sends pending changes and fetches new ones from the server": "odešle čekající změny a stáhne nové ze serveru",
  server: "server",
  settings: "nastavení",
  skipped: "přeskočeno",
  "the server will store your data in plain form again": "server bude tvá data zase ukládat v čitelné podobě",
  "the whole local copy as a file — no network needed": "celá lokální kopie jako soubor — bez sítě",
  today: "dnes",
  unknown: "neznámé",
  "unlock the budget on a new device without the password": "odemknout rozpočet na novém zařízení bez hesla",
  yesterday: "včera",
  "you'll split it into envelopes in the budget": "rozdělíš do obálek v rozpočtu",
  "{amount} from the previous month": "{amount} z předchozího měsíce",
  "{amount} left": "zbývá {amount}",
  "{amount} stays in “To be budgeted”.": "{amount} zůstává v „K rozdělení“.",
  "{amount} to go": "chybí {amount}",
  "{n} OK": "{n} OK",
  "{n} change is waiting to be sent | {n} changes are waiting to be sent": {
    one: "{n} změna čeká na odeslání",
    few: "{n} změny čekají na odeslání",
    many: "{n} změny čeká na odeslání",
    other: "{n} změn čeká na odeslání",
  },
  "{n} day ago | {n} days ago": { one: "před {n} dnem", few: "před {n} dny", many: "před {n} dne", other: "před {n} dny" },
  "{n} hour ago | {n} hours ago": { one: "před {n} hodinou", few: "před {n} hodinami", many: "před {n} hodiny", other: "před {n} hodinami" },
  "{n} item | {n} items": { one: "{n} položka", few: "{n} položky", many: "{n} položky", other: "{n} položek" },
  "{n} minute ago | {n} minutes ago": { one: "před {n} minutou", few: "před {n} minutami", many: "před {n} minuty", other: "před {n} minutami" },
  "{n} near limit": "{n} blízko limitu",
  "{n} over | {n} over": { one: "{n} překročená", few: "{n} překročené", many: "{n} překročené", other: "{n} překročených" },
  "{n} transaction | {n} transactions": { one: "{n} transakce", few: "{n} transakce", many: "{n} transakce", other: "{n} transakcí" },
  "{pct}% of net worth": "{pct} % čistého majetku",
  "{place} — add details": "{place} — doplň podrobnosti",
  "{sym}/mo": "{sym}/měs.",
  Merge: "Sloučit",
  "Look like the same thing": "Vypadají jako totéž",
  "Name after merging": "Název po sloučení",
  "Select these": "Vybrat tyto",
  "Pick at least two entries to merge.": "Vyber alespoň dvě položky ke sloučení.",
  "Merging keeps every transaction — they just move to the entry that stays.": "Sloučení zachová všechny transakce — jen se přesunou k položce, která zůstává.",
  "{n} selected | {n} selected": { one: "vybráno {n}", few: "vybráno {n}", many: "vybráno {n}", other: "vybráno {n}" },
  "Merge {n} entry into one named “{name}”? Every transaction it carries moves over. | Merge {n} entries into one named “{name}”? Every transaction they carry moves over.":
    {
      one: "Sloučit {n} položku do jedné s názvem „{name}“? Všechny její transakce se přesunou.",
      few: "Sloučit {n} položky do jedné s názvem „{name}“? Všechny jejich transakce se přesunou.",
      many: "Sloučit {n} položky do jedné s názvem „{name}“? Všechny jejich transakce se přesunou.",
      other: "Sloučit {n} položek do jedné s názvem „{name}“? Všechny jejich transakce se přesunou.",
    },
  // PR5 Home widgets (attention/recent/spending/goals/trends/heatmap) + their EditWidgetsSheet
  // subtitles — brand-new copy, no prior translation to carry over.
  "Needs attention": "Vyžaduje pozornost",
  "Recent activity": "Nedávná aktivita",
  "When you spend": "Kdy utrácíš",
  "Overspends, pace risks and shortfalls": "Přečerpání, rizika tempa a schodky",
  "6-month sparklines": "Mini grafy za 6 měsíců",
  "daily spending heatmap": "Teplotní mapa denních výdajů",
  "{name} is overspent": "{name}: rozpočet překročen",
  "{n} envelope is overspent | {n} envelopes are overspent": {
    one: "{n} obálka má překročený rozpočet",
    few: "{n} obálky mají překročený rozpočet",
    many: "{n} obálky mají překročený rozpočet",
    other: "{n} obálek má překročený rozpočet",
  },
  "{name} will overspend at this pace": "{name}: tímto tempem překročí rozpočet",
  "Unassigned money is sitting in the pool": "Nepřiřazené peníze čekají ve fondu",
  "You assigned more than you have": "Přiřazeno bylo víc, než máš",
  "{n} goal is short this month | {n} goals are short this month": {
    one: "{n} cíl není tento měsíc naplněn",
    few: "{n} cíle nejsou tento měsíc naplněny",
    many: "{n} cíle nejsou tento měsíc naplněny",
    other: "{n} cílů není tento měsíc naplněno",
  },
  "{name} is in the red": "{name} je v mínusu",
  "{n} account is in the red | {n} accounts are in the red": {
    one: "{n} účet je v mínusu",
    few: "{n} účty jsou v mínusu",
    many: "{n} účty jsou v mínusu",
    other: "{n} účtů je v mínusu",
  },
  "Nothing needs you right now": "Teď nic nevyžaduje pozornost",
  "Cover it ›": "Pokrýt ›",
  "Top up ›": "Dorovnat ›",
  "Suggest ›": "Navrhnout ›",
  "Budget ›": "Rozpočet ›",
  "Fill ›": "Naplnit ›",
  "Reconcile ›": "Spárovat ›",
  "All transactions ›": "Všechny transakce ›",
  "total {amount} this month": "celkem {amount} tento měsíc",
  "No spending this month.": "Žádné výdaje tento měsíc.",
  "Not enough history yet — trends appear after two months of spending.": "Zatím není dost historie — trendy se zobrazí po dvou měsících výdajů.",
  // Wide Home board — edit mode (PR5 Task 6)
  "Add widget": "Přidat widget",
  "Every widget is already on the grid.": "Všechny widgety jsou už v mřížce.",
  "Widget settings": "Nastavení widgetu",
  "latest transactions": "poslední transakce",

  // --- wide-onboarding epic (PR7 task 5, consolidated pass) ---
  "+ {n} more | + {n} more": { one: "+ ještě {n}", few: "+ ještě {n}", many: "+ ještě {n}", other: "+ ještě {n}" },
  "A local copy means the app opens instantly and keeps working on a plane.":
    "Lokální kopie znamená, že se aplikace otevře okamžitě a funguje dál i v letadle.",
  "A negative balance is fine — that is how a credit card starts.": "Záporný zůstatek je v pořádku — tak třeba začíná kreditní karta.",
  "A pairing code moves the key from an unlocked device — paste only, no QR scanning.":
    "Párovací kód přenese klíč z odemčeného zařízení — jen vložení, žádné skenování QR.",
  "A user id can change when a self-hosted server is rebuilt — same e-mail, new account.":
    "ID uživatele se může změnit, když se znovu sestaví samostatně hostovaný server — stejný e-mail, nový účet.",
  "AI through Enveo's key stays off while encryption is on — your own OpenAI key still works, straight from this device.":
    "AI přes klíč Enveo zůstává vypnutá, dokud je zapnuté šifrování — tvůj vlastní klíč OpenAI ale funguje dál, přímo z tohoto zařízení.",
  "Add Enveo to this device": "Přidat Enveo do tohoto zařízení",
  "Choose a report to open it here.": "Vyber report — otevře se tady.",
  "Choose an account to see its details.": "Vyber účet — zobrazí se tu jeho podrobnosti.",
  "Choose an envelope to see its summary.": "Vyber obálku — zobrazí se tu její přehled.",
  "Cover the overspend in {name}": "Pokrýt přečerpání v {name}",
  "Cover {amount}": "Pokrýt {amount}",
  "Covered {amount} in {name}": "Pokryto {amount} v {name}",
  "Details panel": "Panel podrobností",
  "Dismiss this step for now": "Prozatím tento krok přeskočit",
  "Enveo keeps a full local copy of the budget, so it works offline and syncs when you are back. Accounts are mandatory — the copy has to belong to somebody.":
    "Enveo si drží úplnou lokální kopii rozpočtu, takže funguje offline a synchronizuje se, jakmile jsi zpět online. Účty jsou povinné — kopie musí někomu patřit.",
  "Every write to the server is already blocked. Nothing has been deleted, because the decision is yours: the copy on this device may be the last one that exists.":
    "Každý zápis na server je už zablokovaný. Nic nebylo smazáno, protože rozhodnutí je na tobě — kopie na tomto zařízení může být poslední, která existuje.",
  "Export first: the JSON backup is written offline, without the server.": "Nejdřív export: záloha JSON se zapíše offline, bez serveru.",
  "Fill all goals ›": "Naplnit všechny cíle ›",
  "Fill {amount} ›": "Naplnit {amount} ›",
  "Filled {amount} in {name}": "Naplněno {amount} v {name}",
  "Hide the side panel": "Skrýt boční panel",
  Ignore: "Přeskočit",
  In: "Příjem",
  "Language and currency": "Jazyk a měna",
  "Language, one account, a set of envelopes. Everything can be renamed, moved or archived later — nothing here is permanent.":
    "Jazyk, jeden účet, sada obálek. Všechno se dá později přejmenovat, přesunout nebo archivovat — nic tady není natrvalo.",
  "Left over": "Zbývá",
  "Left over = income minus spending that month. Last 12 months, newest first.":
    "Zbývá = příjmy minus výdaje za daný měsíc. Posledních 12 měsíců, nejnovější první.",
  "Met in {n} of 5 months · ✓ = fully funded that month": "Splněno {n} z 5 měsíců · ✓ = ten měsíc plně naplněno",
  "Move the missing amount from To be budgeted into this envelope": "Přesunout chybějící částku z K rozdělení do této obálky",
  "Next day": "Další den",
  "No category": "Bez kategorie",
  "No envelope": "Bez obálky",
  "No group": "Bez skupiny",
  "No place": "Bez místa",
  "No spending this day.": "Tento den žádné výdaje.",
  "Nothing is open in this panel yet.": "V tomto panelu zatím nic není otevřené.",
  "Nothing to fix right now ✓": "Momentálně není co řešit ✓",
  "One click and Enveo runs in its own window — offline, full screen, no browser bar.":
    "Jedno kliknutí a Enveo běží ve vlastním okně — offline, na celou obrazovku, bez lišty prohlížeče.",
  "Open in Transactions ›": "Otevřít v Transakcích ›",
  "Open transactions ›": "Otevřít transakce ›",
  Out: "Výdaj",
  "Past months are judged against today's target — changing a goal rewrites its history.":
    "Minulé měsíce se posuzují podle dnešního cíle — změna cíle přepíše jeho historii.",
  "Pick at least one envelope.": "Vyber aspoň jednu obálku.",
  "Plan closed — every envelope holds": "Plán uzavřen — každá obálka drží",
  "Previous day": "Předchozí den",
  "Removing the local copy is the only destructive action here.": "Odebrání lokální kopie je jediná nevratná akce tady.",
  "Savings rate this month: {pct}%": "Míra úspor tento měsíc: {pct} %",
  "Self-hosted or cloud — the same build, the same data model.": "Vlastní server, nebo cloud — stejný build, stejný datový model.",
  "Show all": "Zobrazit vše",
  "Show the side panel": "Zobrazit boční panel",
  "Sign-in is per device: a shared computer can stay session-only.":
    "Přihlášení je na každém zařízení zvlášť — sdílený počítač může zůstat jen na dobu relace.",
  "The data is encrypted on the device before it reaches the server. Only the encryption password — or a pairing code from a device that is already unlocked — can open it here.":
    "Data se šifrují na zařízení dřív, než se dostanou na server. Otevřít je tady dokáže jen šifrovací heslo — nebo párovací kód z už odemčeného zařízení.",
  "The flat line in each spark marks that envelope's median.": "Vodorovná čára v každém mini grafu značí medián dané obálky.",
  "The server stores ciphertext and never sees the password.": "Server ukládá jen šifrovaná data a heslo nikdy nevidí.",
  "This budget is end-to-end encrypted.": "Tento rozpočet je end-to-end šifrovaný.",
  "This device holds someone else's budget.": "Toto zařízení obsahuje rozpočet někoho jiného.",
  "This step was ignored — click to bring it back": "Tento krok byl přeskočený — klikni pro jeho vrácení",
  "Three steps and the budget is yours.": "Tři kroky a rozpočet je tvůj.",
  "Top up {amount}": "Dorovnat {amount}",
  "Top up {name} — almost at the limit": "Dorovnat {name} — blíží se limitu",
  "Top up {name} — its pace will bust the budget": "Dorovnat {name} — tímto tempem překročí rozpočet",
  "Topped up {amount} in {name}": "Dorovnáno {amount} v {name}",
  Undo: "Zpět",
  "Your budget is ready.": "Tvůj rozpočet je připravený.",
  "Your budget is tied to your account.": "Tvůj rozpočet je vázaný na tvůj účet.",
  "at this pace ≈ {projected} against a {budget} budget": "tímto tempem ≈ {projected} proti rozpočtu {budget}",
  "ignored ↩": "přeskočeno ↩",
  "net per month · scale ±{max}": "čistý tok za měsíc · měřítko ±{max}",
  "vs the {avg} daily average": "vs denní průměr {avg}",
  "{funded} of {target}": "{funded} z {target}",
  "{left} left · {pct}% of the budget spent": "zbývá {left} · vyčerpáno {pct} % rozpočtu",
  "{month} so far: {pct}%": "{month} zatím: {pct} %",
  "{month} · ended at {pct}% of the goal": "{month} · skončil na {pct} % cíle",
  "{month} · goal fully funded ✓": "{month} · cíl plně naplněn ✓",
  "{month} · so far {pct}%": "{month} · zatím {pct} %",
  "{name} moved the most this month — {amount} vs median": "{name} se tento měsíc hnula nejvíc — {amount} vs medián",
  "{name} {pct}%": "{name} {pct} %",
  "{now} · median {median}": "{now} · medián {median}",
  "{n} envelope selected | {n} envelopes selected": {
    one: "{n} obálka vybraná",
    few: "{n} obálky vybrané",
    many: "{n} obálky vybrané",
    other: "{n} obálek vybraných",
  },
  "{n} healthy envelope | {n} healthy envelopes": {
    one: "{n} obálka v pořádku",
    few: "{n} obálky v pořádku",
    many: "{n} obálky v pořádku",
    other: "{n} obálek v pořádku",
  },
  "{n} healthy envelope · {ignored} ignored | {n} healthy envelopes · {ignored} ignored": {
    one: "{n} obálka v pořádku · {ignored} přeskočeno",
    few: "{n} obálky v pořádku · {ignored} přeskočeno",
    many: "{n} obálky v pořádku · {ignored} přeskočeno",
    other: "{n} obálek v pořádku · {ignored} přeskočeno",
  },
  "{n} of {m} envelopes is healthy | {n} of {m} envelopes are healthy": {
    one: "{n} z {m} obálek je v pořádku",
    few: "{n} z {m} obálek jsou v pořádku",
    many: "{n} z {m} obálek jsou v pořádku",
    other: "{n} z {m} obálek je v pořádku",
  },
  "{n} step to a closed plan · {pct}% of the month gone | {n} steps to a closed plan · {pct}% of the month gone": {
    one: "{n} krok do uzavřeného plánu · {pct} % měsíce za námi",
    few: "{n} kroky do uzavřeného plánu · {pct} % měsíce za námi",
    many: "{n} kroky do uzavřeného plánu · {pct} % měsíce za námi",
    other: "{n} kroků do uzavřeného plánu · {pct} % měsíce za námi",
  },
  "{n} transaction · avg {avg} · largest {largest} | {n} transactions · avg {avg} · largest {largest}": {
    one: "{n} transakce · prům. {avg} · největší {largest}",
    few: "{n} transakce · prům. {avg} · největší {largest}",
    many: "{n} transakce · prům. {avg} · největší {largest}",
    other: "{n} transakcí · prům. {avg} · největší {largest}",
  },
  "A source is asked for more than it has": "Po jednom ze zdrojů chcete víc, než má",
  "Amount from {name}": "Částka z {name}",
  "Move {sum}": "Přesunout {sum}",
  "Moving {sum} of {amount}": "Přesouváte {sum} z {amount}",
  "Needs {amount} · pick where it comes from": "Chybí {amount} · vyberte, odkud je vzít",
  "No envelope has money left this month.": "Žádná obálka nemá tento měsíc volné peníze.",
  "Only {available} available": "K dispozici jen {available}",
  "Top up {name}": "Dorovnat {name}",
  savings: "úspory",
  "{amount} to cover · {pool} in To be budgeted": "K pokrytí {amount} · v K rozdělení {pool}",
  "{available} available": "K dispozici {available}",
  "{available} available → {after} left": "K dispozici {available} → zůstane {after}",
};

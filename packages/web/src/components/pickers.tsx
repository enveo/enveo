...
export function ScrollPicker<T extends string | number>({
  ...
  return (
    <div style={{ height: IH * VIS, overflow: "hidden", position: "relative", width: width ?? "auto" }}>
      ...
      <div ref={ref} className="gs" onScroll={onScroll} style={{ height: IH, overflowY: "auto", overscrollBehavior: "contain", touchAction: "pan-y", scrollSnapType: "y mandatory", paddingTop: PADH, paddingBottom: PADH, position: "relative", zIndex: 2, WebkitOverflowScrolling: "touch" }}>
        {items.map((it, i) => {
          const s = it === selected;
          return (
        ...
        {KEYS.map((k, i) => {
          ...
          return (
            <button
              ...
              style={{ padding: "10px 0", background: isOk ? "var(--accent-22)" : op ? C.chip : C.key, border: "none", borderRadius: 10, color: isOk ? TEAL : op ? C.soft : C.text, fontSize: op ? 13.5 : 16, fontWeight: 600, fontFamily: font, cursor: "pointer", boxShadow: op || isOk ? "none" : "0 1px 2px rgba(30,30,40,0.08)" }}
            >
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 1, background: C.keybg, paddingBottom: "env(safe-area-inset-bottom)" }}>
      {KEYS.map((k, i) => (
        <button
          ...
          style={{ padding: "15px 0", background: k[0] === "OK" ? "var(--accent-22)" : k[1] === "o" || k[0] === "DEL" ? C.chip : C.key, border: "none", color: k[0] === "OK" ? TEAL : k[1] === "o" || k[0] === "DEL" ? C.mute : C.text, fontSize: k[0] === "DEL" ? 16 : 19, fontWeight: 500, fontFamily: font, cursor: "pointer" }}
        >
